...
import { DatumCampaigns, DatumPublishers, DatumPauseRegistry } from "../typechain-types";
...
describe("DatumCampaigns", function () {
  ...
  before(async function () {
    ...
    // Deploy campaigns with publishers reference + pause registry
    ...
    // Wire governance and settlement directly
  ...
  it("L5: terminateCampaign slashes 10% to governance and refunds 90% to advertiser", async function () {
    ...
    expect(await campaigns.getCampaignStatus(id)).to.equal(4); // Terminated
    expect(await campaigns.getCampaignRemainingBudget(id)).to.equal(0n);
  ...
  it("L6: expirePendingCampaign works after pendingExpiryBlock", async function () {
    ...
    expect(await campaigns.getCampaignStatus(id)).to.equal(5); // Expired
  ...
  it("L8: exhausting budget auto-completes campaign", async function () {
    ...
    expect(await campaigns.getCampaignStatus(id)).to.equal(3); // Completed
    expect(await campaigns.getCampaignRemainingBudget(id)).to.equal(0n);
  });
  ...
  it("Snapshot: settlement uses snapshotTakeRateBps, not updated rate", async function () {
    ...
    const [,,,,takeRate1] = await campaigns.getCampaignForSettlement(id1);
    expect(takeRate1).to.equal(5000);
    ...
    const [,,,,takeRate2] = await campaigns.getCampaignForSettlement(id2);
    expect(takeRate2).to.equal(8000);

    // First campaign still has 50% snapshot
    const [,,,,takeRate1after] = await campaigns.getCampaignForSettlement(id1);
    expect(takeRate1after).to.equal(5000);
  });
  ...
  it("createCampaign: reverts if bid below minimumCpmFloor", async function () {
    ...
    const highFloor = await HighFloorFactory.deploy(parseDOT("0.1"), PENDING_TIMEOUT, await publishers.getAddress(), await pauseReg.getAddress());
