// deploy-evm.ts — EVM bytecode deployment to Passet Hub (pallet-revive EVM mode)
// Compiles with standard solc (not resolc/pvm). keccak256 hashing, no PolkaVM precompiles.
// pallet-revive accepts both PVM and standard EVM bytecode on Passet Hub.
//
// Usage:
...
const ADDR_FILE = path.join(__dirname, "..", "deployed-addresses-evm.json");
const EXT_ADDR_FILE = path.join(__dirname, "..", "extension", "deployed-addresses-evm.json");
