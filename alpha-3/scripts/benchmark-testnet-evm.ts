 ...
 *
 * NOTE: EVM variant — Passet Hub's SYSTEM precompile at 0x900 is a chain-level
 *       contract, visible to both PVM and EVM contracts. So EVM DatumClaimValidator
 *       always takes the blake256 path. Claim hashes use blake2b via @noble/hashes.
 *       Contracts compiled with solc (not resolc/PVM), deployed-addresses-evm.json.
 *
 * Usage:
...
// ---------------------------------------------------------------------------
// Blake2-256 claim hash (SYSTEM 0x900 precompile is chain-level on Passet Hub)
// ---------------------------------------------------------------------------
...
async function initBlake2(): Promise<void> {
  try {
    ...
    console.log("Blake2-256 loaded (Passet Hub SYSTEM precompile is chain-level, EVM contracts use blake256 too)");
  } catch (e: any) {
...
async function main() {
  ...
  const addrs = JSON.parse(fs.readFileSync(__dirname + "/../deployed-addresses-evm.json", "utf-8"));
  ...
  async function createAndActivate(budget = BUDGET): Promise<bigint> {
    ...
    await sendCall(bob, rawProvider, campaignsAddr, campaignsIface,
      "createCampaign", [diana.address, DAILY_CAP, BID_CPM, [], false, ethers.ZeroAddress, 0n], budget);
  ...
  console.log("1. createCampaign");
  {
    const data = campaignsIface.encodeFunctionData("createCampaign",
      [diana.address, DAILY_CAP, BID_CPM, [], false, ethers.ZeroAddress, 0n]);
