...
|---|---|---|
| **Token plane** | `AssetHubPrecompileMock` + `devnetUnwrapShimEnabled=true` Wrapper; asset id 31337; single MintAuthority | XCM-aware Wrapper (`transferToSubstrate`, shim off), real Asset Hub asset, **parachain issuance pallet** + sunset sequence (`stageIssuerTransfer`→`acceptIssuerRole`, **irrevocable**). PRE-MAINNET §L3/§5.5. |
