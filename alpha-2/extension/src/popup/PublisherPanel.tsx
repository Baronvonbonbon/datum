...
import { getSettlementContract, getPaymentVaultContract, getPublishersContract, getRelayContract, getProvider } from "@shared/contracts";
...
export function PublisherPanel({ address }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const vault = settings.contractAddresses.paymentVault
        ? getPaymentVaultContract(settings.contractAddresses, provider) : null;
      ...
      const [bal, pubData, blockNum, stored] = await Promise.all([
        vault ? vault.publisherBalance(address).catch(() => 0n) : 0n,
