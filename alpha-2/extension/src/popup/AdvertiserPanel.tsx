...
import { getCampaignsContract, getLifecycleContract, getBudgetLedgerContract, getProvider } from "@shared/contracts";
...
export function AdvertiserPanel({ address }: Props) {
        ...
        const results = await Promise.all(
          batch.map(async (id) => {
            try {
              ...
              const remaining = ledger ? await ledger.getRemainingBudget(BigInt(id)).catch(() => 0n) : 0n;
              return {
                id,
                status: Number(settlementData[0]) as CampaignStatus,
                remainingBudget: BigInt(remaining),
                bidCpmPlanck: BigInt(settlementData[2]),
                publisher: settlementData[1] ?? "",
                snapshotTakeRateBps: Number(settlementData[3]),
              } as MyCampaign;
            } catch {
              return null;
            }
  ...
  async function doAction(id: number, action: string) {
    ...
    try {
      ...
      const lifecycle = getLifecycleContract(settings.contractAddresses, signer);
      ...
      switch (action) {
        ...
        case "resume":
          ...
          break;
        case "complete":
          tx = await lifecycle.completeCampaign(BigInt(id));
          break;
        case "expire":
          tx = await lifecycle.expirePendingCampaign(BigInt(id));
          break;
        default:
