...
import { getGovernanceV2Contract, getGovernanceSlashContract, getCampaignsContract, getLifecycleContract, getBudgetLedgerContract, getProvider } from "@shared/contracts";
...
// Alpha-2: Conviction 0-8, escalating cost curve
// Low levels = cheap casual participation, high levels = true conviction
const CONVICTION_LABELS: Record<number, string> = {
  ...
  4: "6x — 21 day lockup",
  5: "9x — 3 month lockup",
  6: "14x — 6 month lockup",
  7: "18x — 9 month lockup",
  8: "21x — 1 year lockup",
};
...
export function GovernancePanel({ address }: Props) {
  ...
  const loadCampaigns = useCallback(async () => {
    ...
    try {
      ...
      const ledger = settings.contractAddresses.budgetLedger
        ? getBudgetLedgerContract(settings.contractAddresses, provider) : null;

      const [nextId, quorum, slash, timeout, blockNum] = await Promise.all([
        campaignsContract.nextCampaignId(),
        v2.quorumWeighted(),
        v2.slashBps(),
        campaignsContract.pendingTimeoutBlocks().catch(() => 0n),
          ...
          batch.map(async (id) => {
            try {
              const [status, advertiser, remaining, ayeW, nayW, resolved] = await Promise.all([
                ...
                ledger ? ledger.getRemainingBudget(BigInt(id)) : 0n,
  ...
  async function expirePending(cid: string) {
    ...
    try {
      const settings = await getSettings();
      const signer = getSigner(settings.rpcUrl);
      const lifecycleContract = getLifecycleContract(settings.contractAddresses, signer);

      const tx = await lifecycleContract.expirePendingCampaign(BigInt(cid));
      await tx.wait();
