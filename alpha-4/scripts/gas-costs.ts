/**
 * gas-costs.ts — Per-function gas (weight) estimates for all alpha-4 contracts
 * ============================================================================
...
// ── Deployed addresses ────────────────────────────────────────────────────────
const addrFileName = "deployed-addresses.json";
...
// ─────────────────────────────────────────────────────────────────────────────
async function main() {
  console.log("\nDatum Alpha-4 — Gas Cost Estimation");
  ...
  // ── Campaigns: Inline Targeting (was TargetingRegistry) ────────────────────
  const TAG_HASH = "0x" + "cd".repeat(32);
  await est("Campaigns", ADDR.campaigns,
    "setPublisherTags(bytes32[] calldata tagHashes)", [[TAG_HASH]], PK.diana,
    "Diana sets 1 tag");
  await est("Campaigns", ADDR.campaigns,
    "setPublisherTags(bytes32[] calldata tagHashes)", [Array(8).fill(TAG_HASH)], PK.diana,
    "Diana sets 8 tags (max for campaign)");

  view("Campaigns", "getPublisherTags2(address) → bytes32[]");
  view("Campaigns", "hasAllTags(address, bytes32[]) → bool");
  ...
  // ── GovernanceV2: Inline Slash (was GovernanceSlash) ────────────────────────
  await est("GovernanceV2", ADDR.governanceV2,
  ...
  view("GovernanceV2", "getClaimable(uint256, address) → uint256");
  ...
  await est("Settlement", ADDR.settlement,
    "setRateLimits(uint256 _windowBlocks, uint256 _maxEventsPerWindow)", [100n, 500_000n], PK.alice,
    "inline rate limiter config");
  ...
  // ── Settlement: Inline Rate Limiter (was DatumSettlementRateLimiter) ──────
  // setRateLimits already estimated above in Settlement section
  view("Settlement", "rlWindowBlocks() → uint256");
