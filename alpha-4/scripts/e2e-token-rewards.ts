...
function claimHash(packed: Uint8Array): string {
...
async function main() {
  ...
  if (!fs.existsSync(addrFile)) { console.error("No deployed-addresses.json"); process.exitCode = 1; return; }
  const A = JSON.parse(fs.readFileSync(addrFile, "utf-8"));

  if (!A.tokenRewardVault) { console.error("tokenRewardVault not in deployed-addresses.json"); process.exitCode = 1; return; }
  ...
  let tokenAddr: string;
  {
    const t0 = Date.now();
    try {
      const artifactPath = path.resolve(__dirname, "../artifacts/contracts/MockERC20.sol/MockERC20.json");
