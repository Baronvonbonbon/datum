// Tests for Blake2-256 hash consistency in claimBuilder and behaviorChain.
...
describe("blake2Hash", () => {
  test("returns 0x-prefixed 32-byte hex string", () => {
    const h = blake2Hash(["uint256"], [42n]);
  ...
  test("deterministic — same inputs produce same hash", () => {
    const a = blake2Hash(["uint256", "address"], [1n, "0x0000000000000000000000000000000000000001"]);
    const b = blake2Hash(["uint256", "address"], [1n, "0x0000000000000000000000000000000000000001"]);
    expect(a).toBe(b);
  });

  test("different inputs produce different hashes", () => {
    const a = blake2Hash(["uint256"], [1n]);
    const b = blake2Hash(["uint256"], [2n]);
    expect(a).not.toBe(b);
  });
